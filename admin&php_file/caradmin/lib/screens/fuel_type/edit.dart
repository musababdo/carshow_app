...
class FuelTypeEdit extends StatefulWidget {
  final List list;
  final int index;
  FuelTypeEdit({required this.list,required this.index});

  @override
  State<FuelTypeEdit> createState() => _FuelTypeEditState();
}

class _FuelTypeEditState extends State<FuelTypeEdit> {
  ...
  String? id;
  Future save() async{
    var url = Uri.parse('http://10.0.2.2/carshow/admin/fuel_type/edit.php');
  ...
  String? _errorMessage(String hint) {
    if(hint=="نوع الوقود بالعربي"){
      return 'هذا الحقل مطلوب';
    }else if(hint=="نوع الوقود بالانجليزي"){
      return 'هذا الحقل مطلوب';
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("نوع الوقود بالعربي");
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("نوع الوقود بالانجليزي");
                          // ignore: missing_return
                        }
                      },
                      controller: name_en,
                      decoration: InputDecoration(
                          hintStyle: TextStyle(color: Colors.blue),
                          hintText: "نوع الوقود بالانجليزي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    Builder(
