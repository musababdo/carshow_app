...
class MechaCondEdit extends StatefulWidget {
  final List list;
  final int index;
  MechaCondEdit({required this.list,required this.index});

  @override
  State<MechaCondEdit> createState() => _MechaCondEditState();
}

class _MechaCondEditState extends State<MechaCondEdit> {
  ...
  String? id;
  Future save() async{
    var url = Uri.parse('http://10.0.2.2/carshow/admin/mechanical_condition/edit.php');
  ...
  String? _errorMessage(String hint) {
    if(hint=="الحاله الميكانيكيه بالعربي"){
      return 'هذا الحقل مطلوب';
    }else if(hint=="الحاله الميكانيكيه بالانجليزي"){
      return 'هذا الحقل مطلوب';
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("الحاله الميكانيكيه بالعربي");
                      ...
                      decoration: InputDecoration(
                          ...
                          hintText: "الحاله الميكانيكيه بالعربي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("الحاله الميكانيكيه بالانجليزي");
                          // ignore: missing_return
                        }
                      },
                      controller: name_en,
                      decoration: InputDecoration(
                          hintStyle: TextStyle(color: Colors.blue),
                          hintText: "الحاله الميكانيكيه بالانجليزي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    Builder(
