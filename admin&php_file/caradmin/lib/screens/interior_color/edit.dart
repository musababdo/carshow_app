...
class InteriorEdit extends StatefulWidget {
  final List list;
  final int index;
  InteriorEdit({required this.list,required this.index});

  @override
  State<InteriorEdit> createState() => _InteriorEditState();
}

class _InteriorEditState extends State<InteriorEdit> {
  ...
  String? id;
  Future save() async{
    var url = Uri.parse('http://10.0.2.2/carshow/admin/interior_color/edit.php');
  ...
  String? _errorMessage(String hint) {
    if(hint=="اللون الداخلي بالعربي"){
      return 'هذا الحقل مطلوب';
    }else if(hint=="اللون الداخلي بالانجليزي"){
      return 'هذا الحقل مطلوب';
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("اللون الداخلي بالعربي");
                      ...
                      decoration: InputDecoration(
                          ...
                          hintText: "اللون الداخلي بالعربي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("اللون الداخلي بالانجليزي");
                          // ignore: missing_return
                        }
                      },
                      controller: name_en,
