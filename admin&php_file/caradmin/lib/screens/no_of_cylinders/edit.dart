...
class NoOfEdit extends StatefulWidget {
  final List list;
  final int index;
  NoOfEdit({required this.list,required this.index});

  @override
  State<NoOfEdit> createState() => _NoOfEditState();
}

class _NoOfEditState extends State<NoOfEdit> {
  ...
  String? id;
  Future save() async{
    var url = Uri.parse('http://10.0.2.2/carshow/admin/no_of_cylinders/edit.php');
  ...
  String? _errorMessage(String hint) {
    if(hint=="عدد الاسطوانات بالعربي"){
      return 'هذا الحقل مطلوب';
    }else if(hint=="عدد الاسطوانات بالانجليزي"){
      return 'هذا الحقل مطلوب';
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("عدد الاسطوانات بالعربي");
                      ...
                      decoration: InputDecoration(
                          ...
                          hintText: "عدد الاسطوانات بالعربي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("عدد الاسطوانات بالانجليزي");
                          // ignore: missing_return
                        }
                      },
                      controller: name_en,
                      decoration: InputDecoration(
                          hintStyle: TextStyle(color: Colors.blue),
                          hintText: "عدد الاسطوانات بالانجليزي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    Builder(
