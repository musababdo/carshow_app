...
class BodyCondEdit extends StatefulWidget {
  final List list;
  final int index;
  BodyCondEdit({required this.list,required this.index});

  @override
  State<BodyCondEdit> createState() => _BodyCondEditState();
}

class _BodyCondEditState extends State<BodyCondEdit> {
  ...
  String? id;
  Future save() async{
    var url = Uri.parse('http://10.0.2.2/carshow/admin/body_condition/edit.php');
  ...
  String? _errorMessage(String hint) {
    if(hint=="الحاله بالعربي"){
      return 'هذا الحقل مطلوب';
    }else if(hint=="الحاله بالانجليزي"){
      return 'هذا الحقل مطلوب';
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("الحاله بالعربي");
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("الحاله بالانجليزي");
                          // ignore: missing_return
                        }
                      },
                      controller: name_en,
