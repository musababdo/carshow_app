...
class SteeringEdit extends StatefulWidget {
  final List list;
  final int index;
  SteeringEdit({required this.list,required this.index});

  @override
  State<SteeringEdit> createState() => _SteeringEditState();
}

class _SteeringEditState extends State<SteeringEdit> {
  ...
  String? id;
  Future save() async{
    var url = Uri.parse('http://10.0.2.2/carshow/admin/steering_side/edit.php');
  ...
  String? _errorMessage(String hint) {
    if(hint=="جهه القياده بالعربي"){
      return 'هذا الحقل مطلوب';
    }else if(hint=="جهه القياده بالانجليزي"){
      return 'هذا الحقل مطلوب';
    }
  }
                ...
                child: Column(
                  children: [
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("جهه القياده بالعربي");
                    ...
                    TextFormField(
                      validator:(value) {
                        if (value!.isEmpty) {
                          return _errorMessage("جهه القياده بالانجليزي");
                          // ignore: missing_return
                        }
                      },
                      controller: name_en,
                      decoration: InputDecoration(
                          hintStyle: TextStyle(color: Colors.blue),
                          hintText: "جهه القياده بالانجليزي"
                      ),
                    ),
                    SizedBox(height: 15,),
                    Builder(
